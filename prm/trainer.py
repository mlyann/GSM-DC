...
class ProcessRewardTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.placeholder_token_id = tokenizer.convert_tokens_to_ids(".")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        batch_indices = torch.tensor([x for x in range(inputs["input_ids"].shape[0])], dtype=torch.int32)
        seq_indices = []

        for idx in batch_indices:
            token_seq = inputs["input_ids"][idx]
            for i in range(token_seq.shape[0] - 1, -1, -1):
                if token_seq[i] != 128001:
                    seq_indices.append(i)
                    break

        seq_indices = torch.tensor(seq_indices, dtype=torch.int32)
